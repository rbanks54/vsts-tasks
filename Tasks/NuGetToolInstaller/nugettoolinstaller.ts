...
function logNugetToolInstallerStartupVariables() {
    try {
        ...
        if (semver.gte(agentVersion, '2.120.0')) {
            ...
            let epId = taskLib.getInput('externalendpoint');
            if (epId) {
                externalendpoint = {
                    feedName: taskLib.getEndpointUrl(epId, true).replace(/\W/g, ''),
                    feedUri: taskLib.getEndpointUrl(epId, true)
                }
            }
            let externalendpoints = taskLib.getDelimitedInput('externalendpoints', ',');
            if (externalendpoints) {
                externalendpoints = externalendpoints.reduce((ary, id) => {
                    let te = {
                        feedName: taskLib.getEndpointUrl(id, true).replace(/\W/g, ''),
                        feedUri: taskLib.getEndpointUrl(id, true)
                    }
                    ary.push(te);
                    return ary;
                }, []);
